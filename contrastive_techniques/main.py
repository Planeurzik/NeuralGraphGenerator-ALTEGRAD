...
from utils import graph_augment, linear_beta_schedule, construct_nx_from_adj, nt_xent_loss, preprocess_dataset
...
parser.add_argument('--epochs-denoise', type=int, default=6000, help="Number of training epochs for the denoising model (default: 100)")
...
class Projector(nn.Module):
    """
    A projection head (MLP) that maps encoder outputs
    to a space suitable for contrastive learning.
    """
    ...
    def forward(self, x):
        return self.net(x)
projector = Projector(input_dim=args.hidden_dim_encoder, hidden_dim=128).to(device)
# For optimization, include projector params
params_to_optimize = list(autoencoder.parameters()) + list(projector.parameters())
...
def contrastive_step(autoencoder, projector, data, drop_edge_prob=0.1, temperature=0.2):
    """
    1) Creates two augmented views of `data`
    2) Encodes them with `autoencoder.encoder`
    3) Passes each encoding through `projector`
    4) Computes contrastive loss via NT-Xent
    """
    # (A) Create two augmented versions
    data_aug1 = graph_augment(data, drop_edge_prob=drop_edge_prob)
    data_aug2 = graph_augment(data, drop_edge_prob=drop_edge_prob)

    # (B) Encode each version (obtain the hidden vector from the GIN encoder)
    z1 = autoencoder.encoder(data_aug1)  # shape [batch_size, hidden_dim]
    z2 = autoencoder.encoder(data_aug2)  # same shape

    # (C) Project to "contrastive space"
    z1_proj = projector(z1)  # shape [batch_size, hidden_dim_proj]
    z2_proj = projector(z2)

    # (D) Contrastive loss
    loss_contrast = nt_xent_loss(z1_proj, z2_proj, temperature=temperature)
...
if args.train_autoencoder:
    ...
    lambda_contrast = 0.1  # <-- or 1.0, or tune as needed
    ...
    for epoch in range(1, args.epochs_autoencoder+1):
        ...
        for data in train_loader:
            ...
            # 1) Compute VGAE's standard reconstruction + KL loss
            loss_vgae, recon, kld = autoencoder.loss_function(data) 
            # (this includes binary cross-entropy adjacency recon + beta*KLD)

            # 2) Compute contrastive loss
            loss_contrast = contrastive_step(
                autoencoder, projector, data, 
                drop_edge_prob=0.1, 
                temperature=0.2
            )

            # 3) Combine them
            total_loss = loss_vgae + lambda_contrast * loss_contrast
        ...
        with torch.no_grad():
            for data in val_loader:
                data = data.to(device)
                loss_vgae, recon, kld = autoencoder.loss_function(data)
                # If you want to also measure contrastive loss on validation, do:
                loss_contrast = contrastive_step(autoencoder, projector, data, drop_edge_prob=0.1)
                total_loss = loss_vgae + lambda_contrast * loss_contrast
                val_loss_all += total_loss.item()
                cnt_val += 1

        val_avg = val_loss_all / cnt_val
        train_avg = train_loss_all / cnt_train

        print(f"Epoch {epoch} | Train Loss: {train_avg:.4f} | Val Loss: {val_avg:.4f}")
        ...
        if val_avg < best_val_loss:
            ...
            }, 'autoencoder_contrastive.pth.tar')
else:
    # Load your pretrained model
    checkpoint = torch.load('autoencoder_contrastive.pth.tar')
...
# Save to a CSV file
with open("output_contrastive.csv", "w", newline="") as csvfile:
