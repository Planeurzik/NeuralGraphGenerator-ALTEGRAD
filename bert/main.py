...
parser.add_argument('--hidden-dim-denoise', type=int, default=776, help="Hidden dimension size for denoising model layers (default: 512)")
...
parser.add_argument('--dim-condition', type=int, default=256, help="Dimensionality of conditioning vectors for conditional generation (default: 128)")

# Number of conditions used in conditional vector (number of properties)
parser.add_argument('--n-condition', type=int, default=776, help="Number of distinct condition properties used in conditional vector (default: 7) - base BERT")
...
if args.train_autoencoder:
    ...
    for epoch in range(1, args.epochs_autoencoder+1):
        ...
        if best_val_loss >= val_loss_all:
            ...
            }, 'autoencoder_bert.pth.tar')
else:
    checkpoint = torch.load('autoencoder_bert.pth.tar', map_location=device)
    ...
    for epoch in range(1, args.epochs_denoise+1):
        ...
        if best_val_loss >= val_loss_all:
            ...
            torch.save({
                ...
                'optimizer' : optimizer.state_dict(),
            }, 'denoise_model_bert.pth.tar')
else:
    checkpoint = torch.load('denoise_model_bert.pth.tar', map_location=device)
...
# Save to a CSV file
with open("output_bert.csv", "w", newline="") as csvfile:
