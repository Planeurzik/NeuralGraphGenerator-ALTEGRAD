...
from denoise_model_cross_attention import DenoiseNN, p_losses, sample
    ...
    for epoch in range(1, args.epochs_denoise+1):
        ...
        if best_val_loss >= val_loss_all:
            ...
            torch.save({
                ...
                'optimizer' : optimizer.state_dict(),
            }, 'denoise_model_ca.pth.tar')
else:
    checkpoint = torch.load('denoise_model_ca.pth.tar', map_location=device)
    denoise_model.load_state_dict(checkpoint['state_dict'])

denoise_model.eval()

denoise_model_train_params = sum(p.numel() for p in denoise_model.parameters() if p.requires_grad)
print("Denoiser params:", denoise_model_train_params)
print("Total number of params:", denoise_model_train_params+autoencoder_model_train_params)

del train_loader, val_loader

# Save to a CSV file
with open("output_ca.csv", "w", newline="") as csvfile:
