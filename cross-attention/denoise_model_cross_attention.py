...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, cond):
        attn_output, _ = self.attention(x.unsqueeze(1), cond.unsqueeze(1), cond.unsqueeze(1))
...
class DenoiseNN(nn.Module):
    def __init__(self, input_dim, hidden_dim, n_layers, n_cond, d_cond):
        ...
        self.cross_attention = CrossAttention(d_model=d_cond, n_heads=8)
        
        self.input_proj = nn.Linear(hidden_dim, d_cond)
        self.output_proj = nn.Linear(d_cond, hidden_dim)
    ...
    def forward(self, x, t, cond):
        ...
        for i in range(self.n_layers-1):
            if i!=0:
                ...
                x_proj = self.cross_attention(x_proj, cond)  # Apply cross-attention
                x = self.output_proj(x_proj)  # Project back to original hidden_dim
