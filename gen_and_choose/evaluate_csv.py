...
csv_file_path = 'output_baseline.csv'  # Update this with your CSV file path
test_file_path = 'data/test/test.txt'  # Update this with your test.txt file path
...
def load_graphs_from_csv(file_path):
    ...
    graphs = []
    for edge_list_str in df['edge_list']:
        # Use regex to extract tuples from the edge list string
        edges = re.findall(r'\((\d+),\s*(\d+)\)', edge_list_str)
        # Convert the tuples to integer pairs
...
# Step 2: Compute statistics for each graph
...
# Step 4: Compute MAE using evaluation_metrics
def compute_mae(y, y_pred):
    _, mae, _ = evaluation_metrics(y, y_pred)
    return mae

# Step 2: Extract numbers from the ground truth text using the provided function
...
def parse_ground_truth(file_path):
    ...
    with open(file_path, 'r') as file:
        for line in file:
            ...
            # We expect the ground truth to have 7 specific values in a particular order
            if len(numbers) >= 7:
                num_nodes = numbers[1]
                num_edges = numbers[2]
                avg_degree = numbers[3]
                num_triangles = numbers[4]
                global_clustering = numbers[5]
                max_k_core = numbers[6]
                num_communities = numbers[7]
                ground_truth.append([
                    num_nodes, num_edges, avg_degree, num_triangles, global_clustering, max_k_core, num_communities
                ])
    return ground_truth

def filter_relevant_stats(stats_list):
    """
    Filter and return only the relevant 7 statistics for each graph:
    [num_nodes, num_edges, avg_degree, num_triangles, global_clustering, max_k_core, num_communities]
    """
    filtered_stats = []
    for stats in stats_list:
        ...
        num_edges = stats[1]
        avg_degree = stats[5]
        num_triangles = stats[7]
        global_clustering = stats[11]
        max_k_core = stats[12]
        num_communities = stats[13]
...
def filter_single_array(stats):
    ...
    filtered_stats = [num_nodes, num_edges, avg_degree, num_triangles, global_clustering, max_k_core, num_communities]
    return filtered_stats


if __name__=="__main__":
    graphs = load_graphs_from_csv(csv_file_path)
    y = parse_ground_truth(test_file_path)
    y_pred = compute_graph_statistics(graphs)
    y_pred = filter_relevant_stats(y_pred)
    mean, std = calculate_mean_std(y)


    mse, mae, norm_error = evaluation_metrics(y, y_pred)


    mse_all, mae_all, norm_error_all = z_score_norm(y, y_pred, mean, std)

    print("MAE for the graphs:", mae_all)